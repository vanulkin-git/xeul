...
private_users = {
    ...
    "usernames": ['nikitak01', 'fagerewaader']  # тут user и без @ 
}
...
async def handle_welcome(user_id: int, chat_id: int, from_user: types.User, reply_photo_method):
    ...
    if not await check_payment(user_id) and str(user_id) not in admin_chat_ids:  
        ...
        await reply_photo_method(
            ...
            caption=" <b>васап май бой, это ботнет хэйла!</b> \n\n Если хочешь уже ебашить акки, покупай подписку. Нажми на кнопку ниже чтобы купить подписку(премиум).\n\n <b>Сабка даст тебе:</b>\n-  Полная защита от сноса через бота\n-  Снос",
            reply_markup=markup,
            parse_mode="HTML"
        )
        return
    ...
    welcome_message = f"""
🌟 <b>Васап май бой, {first_name} {last_name} {username}!</b> 🌟
Это ботнет @xeulweb. ДИСКЛЕЙМЕР: Ботнет писал не я и не кодер какой-то. Это обычный исходник бота, но более улучшенный и оптимизированный & Заряженный сессиями. Спасибо!
"""
...
async def demolition_callback(callback_query: types.CallbackQuery, state: FSMContext):
    await callback_query.answer()
    markup = InlineKeyboardMarkup(row_width=2)
    btn_email_complaint = InlineKeyboardButton('Снос почтой-', callback_data='email_complaint')
    btn_website_complaint = InlineKeyboardButton('Снос через сайт-', callback_data='website_complaint')
    btn_report_message = InlineKeyboardButton('Снос сессиями', callback_data='report_message')
...
async def back_to_main_menu_callback(callback_query: types.CallbackQuery, state: FSMContext):
    await callback_query.answer()
    markup = InlineKeyboardMarkup(row_width=2)
    btn_support = InlineKeyboardButton('Поддежка', callback_data='support')
    btn_demolition = InlineKeyboardButton(' Снос', callback_data='demolition')  
...
async def process_photo_choice_step(message: types.Message, state: FSMContext):
    ...
    if add_photo == 'да':
        ...
        await message.answer('🔢 Введите количество отправок (не больше 200):')
